from app.rikishi.dependencies.sorting import RikishiSortingParams
from app.rikishi.schemas.rikishi import (
    RikishiSchema,
    CreateRikishiSchema,
    UpdateRikishiSchema,
)
...
class RikishiService:
    ...
    ) -> Pagination[RikishiSchema]:
        rikishis, total = self.repo.get_filtered(sorting, pagination)

        response = Pagination[RikishiSchema](
            total=total, skip=pagination.skip, limit=pagination.limit, records=rikishis
        )
